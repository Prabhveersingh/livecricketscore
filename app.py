...
if page == "⚙️ Admin Panel":
        ...
        with tabs[0]:
            ...
            with col1:
                ...
                with st.form("team_a_form"):
                    new_player = st.text_input("Add Player to Team A")
                    if st.form_submit_button("➕ Add Player"):
                        if new_player and new_player not in data["team_a_squad"]:
                            ...
                            st.success(f"Added {new_player} to Team A")
                            st.rerun()
                ...
                if len(available_batsmen) >= 2:
                    ...
                    if st.button("Submit Ball", type="primary", use_container_width=True):
                        ...
                        if bowler not in data["bowling_stats"]:
                            data["bowling_stats"][bowler] = {"o": 0.0, "w": 0, "r": 0, "balls": 0, "maidens": 0}
                        ...
                        if ball_type not in ["Wide", "No Ball"]:
                            ...
                            if data["balls"] % 6 == 0 and data["balls"] > 0:
                                ...
                                over_start_ball = ((data["balls"] - 6) // 6) * 6
                                over_runs = 0
                                # This is simplified - in production you'd track ball-by-ball
                                if data["bowling_stats"][bowler].get("r", 0) == 0:
    ...
    with tab_match:
        ...
        with col_info1:
            ...
            info_data = {
                ...
                "Current Innings": f"{batting_team}",
                "Overs Remaining": f"{data['max_overs'] - data['overs']:.1f}",
                "Wickets Remaining": f"{10 - data['wickets']}",
                "Run Rate": f"{data['run_rate']:.2f}",
                "Extras": data['extras']
            }
            if data["innings"] == 2:
                info_data["Target"] = data["target"]
                info_data["Required Run Rate"] = f"{required_run_rate:.2f}"
            
            df_info = pd.DataFrame(info_data.items(), columns=["Parameter", "Value"])
            st.table(df_info)
    ...
    # Match Summary
    st.subheader("📋 Current Match Summary")
    summary_data = {
        "Total Runs": data["score"],
